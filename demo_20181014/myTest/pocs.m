function [im, kspFull] = pocs( ksp, iter, watchProgress, maxValueIndex, pfDimsion)
...
%
if nargin < 4
    maxValueIndex = 0;  % 如果未出现该变量，则对其进行赋值
    pfDimsion = 0;
end
...
if nnz(smplPtrn) == numel(smplPtrn)     % only the sampled data were passed / |N|umber of |N|on |Z|ero elements
    ...
    [ ksp, pfDim, isUpper, isLower, Nsmp ] = zerofillPFdim( ksp, wasAddedCoilDim , maxValueIndex, pfDimsion);
    if pfDimsion ~= 0
        pfDim = pfDimsion;
    end
    disp(size(ksp));
...
function [ ksp, pfDim, isUpper, isLower, Nsmp ] = zerofillPFdim( ksp, wasAddedCoilDim, maxValueIndex, pfDimsion )
